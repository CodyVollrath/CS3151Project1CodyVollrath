...
public class Board {
	 ...
	 * @param src
	 * @param dest
	 * @return true if the row or col is adjacent. False otherwise
	 */
	private boolean isAdjacent(Position src, Position dest) {
		boolean isSameRowOrCol = src.getRow() == dest.getRow() || src.getCol() == dest.getCol();
		int rowDistance = Math.abs(src.getRow() - dest.getRow());
		int colDistance = Math.abs(src.getCol() - dest.getCol());
		boolean isRowOrColDistanceOne = rowDistance == 1 || colDistance == 1;
